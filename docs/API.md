...
`WG.CommandLayer.StartAutonomous()` requires explicit current single-player/private-test authority. It enables recruitment and factory production, creates an empty receiving force if necessary, and starts map-wide search/attack control when the first military unit becomes available. It replaces any active corridor grant. Returns false while locked. It does not enable the session toggle, inspect hidden enemy locations, or persist authority.
...
`SetAutomaticMapControl(enabled)` changes the saved local automatic-start preference; false also stops current automation. Automatic boot is independent of an explicit StartAutonomous call and runs live local-game checks before enabling the session.

MapControl supplies recurring map-wide objectives from legitimate observations; Startup owns session orchestration only. Officer still owns membership and dispatch, TacticalController supplies immediate recruit catch-up plans, and Orders validates each command. Startup is revoked on explicit delegation stop/cancellation. Merely reaching zero members does not revoke an opted-in startup; it can restart a fresh tactical controller for later recruits.
