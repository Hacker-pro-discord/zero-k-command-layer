...
For each actual factory build option, existing shared role deficit remains the base. The later economy-growth revision removes the square-root unit-price discount for the matrix-covered share, using the same score units as a 100-metal reference unit. The uncovered share retains the legacy score. The adapter adds 1.5 times the price-neutral demand scale multiplied by the weighted `(pair value - 1)` contribution. It divides by the full undecayed observed value (minimum 300), including uncovered names, plus 50 per unknown radar contact. This retains uncovered demand and makes stale sightings less influential. Missing/invalid pairs contribute zero, and values of 1.00 add no matchup bonus; covered role demand still uses the price-neutral comparison. Values outside the supplied 0.05-1.95 range are rejected. All supplied exact values are provisional; no reviewed status is fabricated.
...
All controlled factories share friendly completed/queued role value, updated after each purchase. Economy/recovery builder demand, actual build options, resource funding, idle native queues and manual factory exclusions still take priority. The cheap early opening applies only below five military units with no covered visual matchup; known counter information can activate matrix scoring immediately. Read-only production advice also considers exact pair scores when covered candidates exist. No building or combat orders originate from the data module.
...
- The initial matrix revision passed 33 Lua regression suites, including tests for matrix direction, same-role/same-cost unit choices across four factories, manual factory release, read-only advice, partial coverage, negative deficits, neutral/missing/invalid values, stale contact decay, radar identity refusal, capability fallback and the persisted off switch.
