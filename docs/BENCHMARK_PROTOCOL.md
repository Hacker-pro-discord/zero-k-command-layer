...
**Validation deferred after failed training:** the first corrected full rerun still produced no wins through its first 26 cases. Finish that entire training run, but retain all three holdout maps without performance evaluation. Independent validation is reserved for a later frozen candidate that demonstrates wins across the training terrain families. This is a decision made from training results, not a claim that the original full validation campaign has completed or that the current controller is calibrated.
