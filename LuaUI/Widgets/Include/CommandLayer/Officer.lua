return function(C)
	...
	function A.submit(intent,ids)
		...
		local op=C.registry.newOperation(all); op.plan=plan; op.command=intent.command or Spring.Utilities.CMD.RAW_MOVE; op.options=C.U.options(intent.options); op.mode=C.settings.privateSession and C.settings.mode or 'ARRIVAL'; op.state='EXECUTING'
		...
		if op.mode=='ARRIVAL' then C.registry.finish(op) end; return op.id
	end
			...
			if op.active then
				...
				for _,id in ipairs(live) do
					...
					if release then
						...
						elseif baseIndex==1 and anchor then
