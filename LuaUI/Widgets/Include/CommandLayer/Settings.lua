return function(U)
	local S={version=1,formation='OFF',mode='LOOSE',spacing=64,rankGap=100,supportDepth=200,skirmDepth=250,artilleryDepth=350,autoAssign=false,autoPlay=true,autoEconomy=true,reservePercent=20,intelHalfLife=90,constructors=false,overlays=true,scale=1,x=30,y=240,privateSession=false,debug=false,override='release',proposalLifetime=60,suggestionInterval=10}
	function S.load(data)
		...
		for _,k in ipairs({'autoPlay','autoEconomy','autoAssign','constructors','overlays','debug'}) do if type(data[k])=='boolean' then S[k]=data[k] end end
