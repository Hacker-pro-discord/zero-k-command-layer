...
return function(C)
	local R={enabled=false,workers={},excluded={},assets={},ignored={},missing={},sites={},requests={},tasks={},retry={},last=-100,status='Recovery idle'}
	...
	function R.release(id) if R.workers[id] then R.workers[id]=nil; R.tasks[id]=nil end; R.excluded[id]=true end
	...
	function R.workerNeed()
		...
		for _,id in ipairs(Spring.GetTeamUnits(Spring.GetMyTeamID()) or {}) do if C.U.owned(id) then
			...
			if R.workers[id] or def.name=='cloakcon' and not R.excluded[id] and #(Spring.GetCommandQueue(id,1) or {})==0 then count=count+1 end
			if v.mobile and not v.builder then military=military+1 end
			if def.isFactory and Spring.GetFactoryCommands then for _,q in ipairs(Spring.GetFactoryCommands(id,-1) or {}) do if q.id and q.id<0 and UnitDefs[-q.id] and UnitDefs[-q.id].name=='cloakcon' then count=count+(q.params and tonumber(q.params[1]) or 1) end end end
		end end
	...
	function R.hold(p,now)
		...
		for _,site in ipairs(R.sites) do if not site.finished and C.U.distance(site.point,p)<900 then
			if now-site.lastThreat>200 then site.finished=true; R.status='Recovery site timed out; pending build requests retained'; return false end
			return true
		end end
		return false
	end
	function R.update()
		...
		for id in pairs(R.workers) do
			if not C.U.owned(id) then R.workers[id]=nil; R.tasks[id]=nil else
				...
				if position and C.observations.nearCombat(position,600) and (not task or task.cmd~=Spring.Utilities.CMD.RAW_MOVE or now-task.time>=10) then
					...
					if danger then
						...
						if task then R.retry[task.key]=now+20 end
				...
				if task and #q>0 then
					...
					if now-(task.progress or task.time)>25 then stalled=true; R.retry[task.key]=now+2; R.release(id); C.debug.log('RECOVERY STALLED','Builder '..id..' made no movement/work progress for 25 seconds; released for manual review. Job can pass to another builder.') end
				end
				if not stalled and not Spring.GetUnitTransporter(id) and Spring.GetUnitRulesParam(id,'retreat')~=1 then
		...
		for _,id in ipairs(available) do
			...
			for _,asset in ipairs(damaged) do for _,site in ipairs(R.sites) do
				...
				if not site.finished and now-site.lastThreat>=20 and C.U.distance(site.point,asset.point)<1000 and not C.observations.nearCombat(asset.point,700) and not occupied[k] and now>=(R.retry[k] or 0) then selected={cmd=CMD.REPAIR,params={asset.id},key=k}; break end
			...
			if not selected then for _,job in ipairs(jobs) do
				...
				if clear and canBuild(id,job.def) and not occupied[k] and now>=(R.retry[k] or 0) then
					if job.target and C.U.owned(job.target) then selected={cmd=CMD.REPAIR,params={job.target},key=k}
					elseif not Spring.GetPositionLosState(job.point[1],job.point[2],job.point[3]) then selected={cmd=Spring.Utilities.CMD.RAW_MOVE,params=job.point,key='survey:'..k}
			...
			if not selected then for _,site in ipairs(R.sites) do local p=site.point; local k='survey:'..key(0,p); if not site.finished and now-site.lastThreat>=20 and not C.observations.nearCombat(p,900) and not Spring.GetPositionLosState(p[1],p[2],p[3]) and not occupied[k] and now>=(R.retry[k] or 0) then selected={cmd=Spring.Utilities.CMD.RAW_MOVE,params=p,key=k}; break end end end
			if not selected and Spring.GetFeaturesInRectangle then for _,site in ipairs(R.sites) do
				if not site.finished and now-site.lastThreat>=20 and not C.observations.nearCombat(site.point,900) then
