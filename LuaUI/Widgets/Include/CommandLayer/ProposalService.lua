return function(C)
	...
	function P.create(f,kind,plan,reason)
		...
		P.nextID=P.nextID+1
		local p={id=P.nextID,revision=1,forceID=f.id,forceRevision=f.revision,kind=kind,plan=C.U.copy(plan),units=C.officer.members(f),generations={},positions={},health={},state='OFFERED',created=C.U.now(),expires=C.U.now()+15,reason=reason,mode=C.settings.mode}
	...
	function P.firstOffered()
		for id=1,P.nextID do local p=P.items[id]; if p and p.state=='OFFERED' then return p end end
	end
	function P.update()
