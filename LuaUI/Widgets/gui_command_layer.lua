...
function widget:Initialize()
	...
	C.recovery=module('Recovery')(C); C.enemyModel=module('EnemyModel')(C); C.productionControl=module('ProductionController')(C); C.startup=module('Startup')(C); C.mapControl=module('MapControl')(C)
	C.rules=module('TacticalRules')(C); C.retreatPriority=module('RetreatPriority')(C); C.routing=module('Routing')(C); C.defense=module('Defense')(C); C.tactical=module('TacticalController')(C)
...
function widget:UnitCommand(id,def,team,cmd,params,opts,tag,player,fromSynced,fromLua)
	if C.orders and not C.orders.event(id,cmd,params,fromSynced) then if C.productionControl then C.productionControl.release(id) end; if C.recovery then C.recovery.release(id) end; if C.arsenal then C.arsenal.release(id) end; C.registry.release({id},'EXTERNAL_ORDER') end
end
...
function widget:Update(dt) if not ready and C.ui then ready=C.ui.initialize() end; if ready then C.logistics.update(); C.observations.update(); C.enemyModel.update(); C.startup.boot(); C.startup.update(); C.recovery.update(); C.officer.update(); C.proposals.update(); C.advisor.update(); C.tactical.update(); C.productionControl.update(); C.ui.update(dt) end end
...
function widget:UnitDestroyed(id) if C.enemyModel then C.enemyModel.destroyed(id) end; if C.recovery then C.recovery.destroyed(id) end; if C.registry then C.registry.release({id},'UNIT_LOST') end end
function widget:UnitTaken(id) if C.recovery then C.recovery.forget(id); C.recovery.release(id) end; if C.registry then C.registry.release({id},'TRANSFERRED') end end
...
function widget:Shutdown() if C.productionControl then C.productionControl.set(false) end; if C.officer then C.officer.cancelAll() end; if C.ui then C.ui.shutdown() end; WG.CommandLayer=nil end
