...
function widget:Initialize()
	...
	C.productionControl=module('ProductionController')(C); C.startup=module('Startup')(C)
...
function widget:Update(dt) if not ready and C.ui then ready=C.ui.initialize() end; if ready then C.logistics.update(); C.observations.update(); C.startup.update(); C.officer.update(); C.proposals.update(); C.advisor.update(); C.tactical.update(); C.productionControl.update(); C.ui.update(dt) end end
