...
function widget:SetConfigData(data) C.settings.load(data); options.suspend.value=C.settings.override=='suspend'; options.autoPlay.value=C.settings.autoPlay; options.autoEconomy.value=C.settings.autoEconomy; for k,o in pairs(options) do if o.type=='number' then o.value=C.settings[k] end end end
...
function widget:Initialize()
	...
	C.economy=module('Economy')(C); C.arsenal=module('Arsenal')(C); C.recovery=module('Recovery')(C); C.enemyModel=module('EnemyModel')(C); C.productionControl=module('ProductionController')(C); C.startup=module('Startup')(C); C.mapControl=module('MapControl')(C)
