...
function widget:SetConfigData(data) C.settings.load(data); options.suspend.value=C.settings.override=='suspend'; options.autoPlay.value=C.settings.autoPlay; for k,o in pairs(options) do if o.type=='number' then o.value=C.settings[k] end end end
...
function widget:Initialize()
	...
	C.arsenal=module('Arsenal')(C); C.recovery=module('Recovery')(C); C.enemyModel=module('EnemyModel')(C); C.productionControl=module('ProductionController')(C); C.startup=module('Startup')(C); C.mapControl=module('MapControl')(C)
...
function widget:CommandNotify(id,params,opts)
	...
	if C.registry and not C.orders.sending then for _,unit in ipairs(Spring.GetSelectedUnits()) do if C.productionControl then C.productionControl.release(unit) end; if C.recovery then C.recovery.release(unit) end; if C.arsenal then C.arsenal.release(unit) end end; C.registry.release(Spring.GetSelectedUnits(),'PLAYER_OVERRIDE') end
	if C.logistics then C.logistics.notify(id,params,opts) end; return false
end
function widget:UnitCommandNotify(id) if C.registry and not C.orders.sending then if C.productionControl then C.productionControl.release(id) end; if C.recovery then C.recovery.release(id) end; if C.arsenal then C.arsenal.release(id) end; C.registry.release({id},'PLAYER_OVERRIDE') end; return false end
function widget:UnitCommand(id,def,team,cmd,params,opts,tag,player,fromSynced,fromLua)
	...
	if C.orders and not ours and not nativeStockpile then if C.productionControl then C.productionControl.release(id) end; if C.recovery then C.recovery.release(id) end; if C.arsenal then C.arsenal.release(id) end; C.registry.release({id},'EXTERNAL_ORDER') end
end
...
function widget:Update(dt) if not ready and C.ui then ready=C.ui.initialize() end; if ready then C.logistics.update(); C.observations.update(); C.enemyModel.update(); C.startup.boot(); C.startup.update(); C.recovery.update(); C.arsenal.update(); C.officer.update(); C.proposals.update(); C.advisor.update(); C.tactical.update(); C.productionControl.update(); C.ui.update(dt) end end
...
function widget:UnitDestroyed(id) if C.arsenal then C.arsenal.release(id) end; if C.enemyModel then C.enemyModel.destroyed(id) end; if C.recovery then C.recovery.destroyed(id) end; if C.registry then C.registry.release({id},'UNIT_LOST') end end
function widget:UnitTaken(id) if C.arsenal then C.arsenal.release(id) end; if C.recovery then C.recovery.forget(id); C.recovery.release(id) end; if C.registry then C.registry.release({id},'TRANSFERRED') end end
...
function widget:Shutdown() if C.arsenal then C.arsenal.stop() end; if C.productionControl then C.productionControl.set(false) end; if C.officer then C.officer.cancelAll() end; if C.ui then C.ui.shutdown() end; WG.CommandLayer=nil end
