...
options.multiplayer={name='Enable full Multiplayer AI for this match',type='button',OnChange=function() if C.officer and C.officer.setMultiplayerSession(true) then C.startup.start() end end}
...
options.autoStructures={name='Automatic Defence / Special support structures',type='bool',value=true,OnChange=function(self) C.settings.autoStructures=self.value end}
options.autoEconomy={name='Automatic economic expansion in authorized sessions',type='bool',value=true,OnChange=function(self) C.settings.autoEconomy=self.value; if C.economy then if self.value then C.economy.start() else C.economy.stop() end end end}
...
function widget:SetConfigData(data) C.settings.load(data); options.suspend.value=C.settings.override=='suspend'; options.autoPlay.value=C.settings.autoPlay; options.autoEconomy.value=C.settings.autoEconomy; options.autoStructures.value=C.settings.autoStructures; options.unitMatchups.value=C.settings.unitMatchups; for k,o in pairs(options) do if o.type=='number' then o.value=C.settings[k] end end end
...
function widget:UnitCommand(id,def,team,cmd,params,opts,tag,player,fromSynced,fromLua)
	...
	if C.orders and not ours and (cmd==Spring.Utilities.CMD.UNIT_SET_TARGET or cmd==Spring.Utilities.CMD.UNIT_CANCEL_TARGET) then C.orders.focus[id]=nil end
...
function widget:UnitDamaged(id,def,team,damage,paralyzer,weapon,projectile,attacker) if C.observations then C.observations.damaged(id,attacker) end end
