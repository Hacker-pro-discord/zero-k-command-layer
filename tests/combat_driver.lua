...
function widget:Update()
	...
	if fid and now-last>=(Spring.GetModOptions().cl_test_startup=='1' and 2 or 10) then
		...
		if d then
			for _,group in ipairs({'SCOUT','RAID','MAIN','RESERVE','DEFENSE','AIR','SEA'}) do local n=0; for _,id in ipairs(d.groups[group] or {}) do if f.members[id] and Spring.ValidUnitID(id) then n=n+1 end end; Spring.Echo('[CL-COMBAT-CLIENT] group='..group..' alive='..n..' op='..tostring(d.ops[group])) end
			...
			if Spring.GetModOptions().cl_test_domains=='1' then for _,group in ipairs({'AIR','SEA'}) do local moved,bad=0,0; for _,id in ipairs(d.groups[group] or {}) do local x,_,z=Spring.GetUnitPosition(id); local old=initial[id]; if x and old and (x-old[1])^2+(z-old[2])^2>32^2 then moved=moved+1 end; local q=(Spring.GetCommandQueue(id,1) or {})[1]; if group=='SEA' and q and q.id==CMD.FIGHT and #q.params>=3 and Spring.GetGroundHeight(q.params[1],q.params[3])>=0 then bad=bad+1 end end; Spring.Echo('[CL-DOMAIN] '..group..' moved='..moved..' invalid_land_destinations='..bad) end end
