...
def analyze(directory, output):
    ...
    for path in sorted(directory.glob('*/summary.json')):
        ...
        if sum(v for k,v in defense.items() if k in ('DEFENDING','RECOVERING'))>len(clients)*.6: notes.append('Defense/recovery occupied over 60% of samples; possible response saturation.')
        ...
        rows.append(row)
        detail=dict(row,diagnosticIndicators=notes,states=dict(states),defenseStates=dict(defense),lossesByUnit=dict(lost),finalOwn=own,finalEnemy=enemy)
